...
import AdminListTable from '../../../components/admindashboard/admin/AdminListTable';
import FilterSection from '../../../components/admindashboard/admin/FilterSection';
import CreateAdmin from '../../../components/admindashboard/admin/CreateAdmin';
...
const List = ({isPermission}) => {
  let localFields = JSON.parse(localStorage.getItem("AdminTableSetting"))
  ? JSON.parse(localStorage.getItem("AdminTableSetting"))
  : Config.AdminTableSetting;

  if (!localStorage.getItem("AdminTableSetting")) {
    localStorage.setItem("AdminTableSetting", JSON.stringify(localFields));
  }

  let localParentData = JSON.parse(localStorage.getItem("AdminParentData"))
  ? JSON.parse(localStorage.getItem("AdminParentData"))
  : null;

  if (!localStorage.getItem("AdminParentData")) {
    localStorage.setItem("AdminParentData", JSON.stringify(localParentData));
  }
  ...
  useEffect(() => {
    ...
    async function fetchTableSetting() {
      await tablesetting({ admin_id: admin_id, role: role, page: "admin" }, onSettingSuccess);
    }
    const request = {
      fields: columns,
      perPage: perPage,
  ...
  /* onRoleListSuccess Response  */
  const onRoleListSuccess = async (res) => {
    setRoleList(res);
  };
  ...
  const handleAddEditToggle = async (id) => {
    if (id) {
      ...
      await getRoleList({role_type:4}, onRoleListSuccess);
    }else{
      setEdit(false);
      setInput({});
      await getRoleList({role_type:4}, onRoleListSuccess);
  ...
  const onSettingSuccess = async (res) => {
    ...
    localStorage.setItem("AdminTableSetting", JSON.stringify(res));
    apiCallStatus();
  };
  ...
  const handleClearFilter = () => {
    setFilters({status: [1,2,3]});
    setEmptyFilter({status: [1,2,3]});
    setRows([]);
  }
  ...
  const handleExportData = async () => {
    ...
    await exportList(payload, onSuccessExport);
  };

  /* On Success Export Get Response */
  const onSuccessExport = async (res) => {
    // console.log(res);
  } ;
  ...
  const handleDuplicate = async (id) => {
    if (id) {
      const payload = {id: id, admin_id: admin.id, parent_id: parentId};
