...
const Datarow = ({rows, columns, handleStatusAction, handleAddEditToggle, isPermission, handleDuplicate,handleUpdatePermissionToggle, handleUserList, handleAdminList, setRowsId, handleEditContent, handleEditComponent}) => {
  const datarows = rows.map((item, i ) => {
    const row = columns.map((col_item, col_i) => {
      if(col_item['field']==="status"){
        if(item['status']===Const.Active){
          return <td><span className="badge text-bg-success">Published</span></td>
        }else if(item['status']===Const.Inactive){
          return <td><span className="badge text-bg-danger">Unpublished</span></td>
      ...
      if(col_item['field']==="status"){
        return (
          ...
          {item['status']!== Const.Active && isPermission?.Active ? 
            <li>
                <button className="dropdown-item" type="button" onClick={() => handleStatusAction(item.id, Const.Active)}><FiCheckCircle style={{color: "#079640", width: "22px", height: "22px", marginRight: "6px"}}/> Publish</button>
            </li>
          :""
          }
          {item['status']!== Const.Inactive && isPermission?.Inactive ? 
            <li>
                <button className="dropdown-item" type="button" onClick={() => handleStatusAction(item.id, Const.Inactive)}><FiSlash style={{color: "#ea3939", width: "22px", height: "22px", marginRight: "6px"}}/> Unpublish</button>
            </li>
          :""
          }
      ...
      <td>
        <div className="btn-group" id="ellipsis">
          ...
          <ul className="dropdown-menu dropdown-menu-lg-end">
            ...
            {isPermission?.Edit ? 
              <li>
                  <button className="dropdown-item" type="button" onClick={() => handleEditComponent(item.id)}><FiEdit style={{color: "#0d7bdb", width: "20px", height: "20px", marginRight: "7px"}}/> Edit Component</button>
              </li>
            :""
            }
            {isPermission?.Edit ? 
              <li>
                  <button className="dropdown-item" type="button" onClick={() => handleEditContent(item.id)}><FiEdit style={{color: "#0d7bdb", width: "20px", height: "20px", marginRight: "7px"}}/> Edit Content</button>
              </li>
            :""
            }
            {CurrentStauts}
